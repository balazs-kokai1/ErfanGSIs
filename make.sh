...
# System tree thing
outputtreename="GSI-Workshop[$romtypename]-[$codename]-[GSI+SGSI]-[$displayid]-[$sourcever]-[$date-$hashdate]-System-Tree.txt"
...
if [ "$outputtype" == "Aonly" ]; then
    if [[ ! $(ls "$systemdir/system/etc/init/" | grep *environ*) ]]; then
        ...
        echo "# AUTOGENERATED FILE BY GSI-Workshop TOOLS" > "$systemdir/system/etc/init/init.treble-environ.rc"
...
outputodmoverlaysname="GSI-Workshop[$romtypename]-[$codename]-[GSI+SGSI]-[$displayid]-[$sourcever]-[$date-$hashdate]-ODMOverlays.tar.gz"
