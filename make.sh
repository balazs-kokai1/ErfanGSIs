...
# System tree thing
outputtreename="PriiiiyoGSI[$romtypename]-[$codename]-[GSI+SGSI]-[$displayid]-[$sourcever]-[$date-$hashdate]-System-Tree.txt"
...
if [ "$outputtype" == "Aonly" ]; then
    if [[ ! $(ls "$systemdir/system/etc/init/" | grep *environ*) ]]; then
        ...
        echo "# AUTOGENERATED FILE BY PRIIIIYOGSI TOOLS" > "$systemdir/system/etc/init/init.treble-environ.rc"
...
# Out info
outputname="PriiiiyoGSI[$romtypename]-[$codename]-[GSI+SGSI]-[$displayid]-[$outputtype]-[$sourcever]-[$date-$hashdate]"
outputimagename="$outputname".img
outputtextname="$outputname".txt
outputvendoroverlaysname="PriiiiyoGSI[$romtypename]-[$codename]-[GSI+SGSI]-[$displayid]-[$sourcever]-[$date-$hashdate]-VendorOverlays.tar.gz"
outputodmoverlaysname="PriiiiyoGSI[$romtypename]-[$codename]-[GSI+SGSI]-[$displayid]-[$sourcever]-[$date-$hashdate]-ODMOverlays.tar.gz"
